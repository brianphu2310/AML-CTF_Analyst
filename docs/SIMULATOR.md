...
The Simulator has two modes, switched at the top right of the page.

* **Case decision** (default) helps the officer decide what to do with one case. Load an alert or type a
  case, tick the red flags and the verification, and get a scored recommendation, the reporting deadlines,
  what would change the decision and the precedent. Then escalate to the MLRO and download the SMR draft
  (Word). The rules are in [METHODOLOGY.md §4](METHODOLOGY.md#4-case-decision-scoring).
* **Alert-model tuning** covers the model-governance question: what happens to volume, false positives and
  workload if the monitoring rules change. It is described below.
