"""Simulator — the flagship page, in two modes.

* Case decision (default, views/case_sim.py): the officer enters a case and gets a recommended decision,
  escalates to the MLRO and downloads an SMR draft.
* Alert-model tuning (below): the model-governance view.

Alert Threshold & Model Tuning:
...
MODES = ["Case decision", "Alert-model tuning"]
HERO = {
    "Case decision": ("FLAGSHIP · DECISION SUPPORT", "Case Decision Simulator",
                      "Enter or load a case, tick what you found, and get a scored recommendation, the regulatory clocks it starts, "
                      "what would change the decision — then escalate to the MLRO and download an SMR draft in Word.",
                      "Analyst → MLRO → AUSTRAC"),
    "Alert-model tuning": ("MODEL GOVERNANCE", "Alert Threshold &amp; Model Tuning",
                           "Tune the 4-factor risk-model weights and the five monitoring-rule thresholds, then see the projected effect "
                           "on alert volume, false positives, analyst hours, SLA-breach risk and detection coverage before anything ships.",
...
def _keep_mode():
    ss = st.session_state
    if ss.get("sim_mode") is None:
        ss["sim_mode"] = ss.get("_sim_mode_last", MODES[0])
    ss["_sim_mode_last"] = ss["sim_mode"]


def render(M, cur, cmp):
    ui.restore_state({"sim_mode": MODES[0]})
    mode = st.session_state.get("sim_mode") or MODES[0]
    kicker, title, sub, tag = HERO[mode]
    h1, h2 = st.columns([3.2, 1], vertical_alignment="center")
    with h1:
        st.markdown(f'<div class="sim-hero"><div><div class="sim-hero-kicker">{kicker}</div><div class="sim-hero-title">{title}</div>'
                    f'<div class="sim-hero-sub">{sub}</div></div><span class="sim-hero-tag">{tag}</span></div>', unsafe_allow_html=True)
    with h2:
        st.segmented_control("Mode", MODES, key="sim_mode", on_change=_keep_mode, label_visibility="collapsed", width="stretch")
    ui.keep_state(["sim_mode"])
    if mode == "Case decision":
        from views import case_sim
        case_sim.render(M)
    else:
        render_tuning(M, cur, cmp)


def render_tuning(M, cur, cmp):
