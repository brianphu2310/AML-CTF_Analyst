...
with c2:
    ...
    kpi_card("High / Critical Risk", f"{high_risk:,}", f"{high_risk/len(customers)*100:.1f}% of book", "#F87171")
with c3:
    open_hits = screening[screening["status"].isin(["Open", "Escalated", "Under Review"])].shape[0]
    kpi_card("Open Screening Alerts", f"{open_hits:,}", f"{screening.shape[0]} total hits", "#FBBF24")
with c4:
    open_cases = cases[~cases["status"].str.contains("Closed")].shape[0]
    kpi_card("Active Cases", f"{open_cases:,}", f"{cases.shape[0]} total this period", "#60A5FA")
with c5:
    pending_smr = cases[cases["status"] == "Pending SMR Lodgement"].shape[0]
    kpi_card("Pending SMR Lodgement", f"{pending_smr:,}", "within statutory 3-day window", "#34D399")
...
with col2:
    ...
    fig2 = px.bar(
        ...
        color=industry_risk.values, color_continuous_scale=["#34D399", "#FBBF24", "#F87171"],
    )
    fig2.update_layout(
...
fig3 = px.area(
    ...
    color_discrete_map={"Inbound": "#2DD4BF", "Outbound": "#60A5FA"},
...
fig4.update_traces(marker_color="#2DD4BF")
fig4.update_layout(
    template="plotly_dark", plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
...
st.info(
    "Use the sidebar to navigate: **Customer Risk**, **Screening**, **Transaction Monitoring**, "
    "**Case Management & SMR**, **UBO Network**, and **Business KYC**.",
    icon="🧭",
)
