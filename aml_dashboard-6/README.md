...
| `pages/8_Alerts_Triage.py` | Unified alert queue (screening hits + transaction-monitoring detections) with analyst disposition and one-click escalation to a case |
| `pages/9_Periodic_Review.py` | Risk-based periodic review scheduling and remediation (KYC refresh, re-screening, risk re-rating) |
| `pages/10_Audit_Trail.py` | Consolidated, filterable, exportable record-keeping log of every decision made across the suite |
| `pages/11_Process_Map.py` | Full 26-stage AML/CTF lifecycle map, each stage linked to the page that implements it |

## Full lifecycle coverage

This suite implements the complete customer lifecycle end to end - onboarding & CDD, ongoing/transaction
monitoring, alert generation & triage, investigation, source of funds/wealth review, escalation, senior
review & decision, SMR assessment/preparation/submission, post-SMR monitoring, periodic review &
remediation, and record keeping/audit trail. See `pages/11_Process_Map.py` for the full stage-by-stage
breakdown and links into the pages that implement each one.

Case status flows through: `Open -> Under Investigation -> Escalated - Senior Review -> Pending SMR
Lodgement -> SMR Lodged -> Post-SMR Monitoring -> Closed (No Action / Restricted-Exited)` - see
`workflow_utils.py` for the state machine, and `4_Case_Management_SAR.py` for the workspace that drives it.

Because this is a demo app with no backing database write layer, in-session actions (alert triage, case
escalation, senior decisions, SMR lodgement, periodic reviews) are held in `st.session_state` and every one
of them is written to the audit trail (`workflow_utils.log_audit`), so `10_Audit_Trail.py` reflects a
realistic combination of seeded history and this session's own actions.
