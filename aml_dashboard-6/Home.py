...
# Extended palette for 3D / multi-series
CHART_SEQ_EXT = [
...
def inject_css():
    ...
    st.markdown(f'<div class="ampl-map-bg">{map_svg}</div>', unsafe_allow_html=True)
    css = _CSS_TEMPLATE.format(
        TEXT_PRIMARY=TEXT_PRIMARY, TEXT_MUTED=TEXT_MUTED,
        BG_GRADIENT_1=BG_GRADIENT_1, BG_GRADIENT_2=BG_GRADIENT_2,
        CARD_BG=CARD_BG, CARD_BORDER=CARD_BORDER, CARD_SHADOW=CARD_SHADOW,
        TEAL_DARK=TEAL_DARK, TEAL_MID=TEAL_MID, TEAL_LIGHT=TEAL_LIGHT,
        TEAL_PALE=TEAL_PALE, TEAL_SOFT=TEAL_SOFT,
        CRITICAL=CRITICAL, HIGH=HIGH, MEDIUM=MEDIUM, LOW=LOW, INFO=INFO,
        CHART_GRID=CHART_GRID, MAP_OPACITY=MAP_OPACITY,
    )
...
def chart_layout_2d(height: int = 340, title: str = "") -> dict:
    """Base layout for 2D charts: transparent bg, pale gridlines."""
    layout = dict(
...
# Alias giữ tương thích với các file cũ dùng plotly_layout_defaults()
...
# Risk-level color map (dùng cho tất cả chart có phân loại risk)
RISK_COLOR_MAP = {
    "Low":      LOW,
    "Medium":   MEDIUM,
    "High":     HIGH,
...
def apply_shadow(fig, marker=True, bar=True):
    """Thêm shadow/gradient nhẹ cho chart 2D để có depth."""
    if bar:
        fig.update_traces(
